...
*/
  ...
  /* 437x240 */
/*
  data->pipeline = gst_parse_launch("udpsrc port=5000 caps=\"application/x-rtp, media=video, clock-rate=90000, encoding-name=H264, sprop-parameter-sets=\\\"J2QAFawrQOD/PwDxImo\\\\=\\\\,KO4fLA\\\\=\\\\=\\\"\", payload=96\" ! gstrtpjitterbuffer ! rtph264depay ! ffdec_h264 ! decodebin2 ! eglglessink sync=false", &error);
*/
...
*/
  /*data->pipeline = gst_parse_launch("udpsrc caps=\"application/x-rtp, media=(string)video, clock-rate=(int)90000, encoding-name=(string)H264, sprop-parameter-sets=(string)\"J2QAFKwrQKD9APEiag,KO4fLA\", ssrc=(uint)2607831725, payload=(int)96, clock-base=(uint)1706962490, seqnum-base=(uint)15791\" port=5000 ! queue ! rtpvh264depay  ! queue ! ffdec_h264 ! autovideosink", &error);
  */
  /*data->pipeline = gst_parse_launch("udpsrc caps=\"application/x-rtp, media=(string)video, clock-rate=(int)90000, encoding-name=(string)RAW, sampling=(string)YCbCr-4:2:0, depth=(string)8, width=(string)320, height=(string)240, colorimetry=(string)SMPTE240M, ssrc=(uint)1825678493, payload=(int)96, clock-base=(uint)2236518695, seqnum-base=(uint)64927\" port=9996 ! queue ! rtpvrawdepay  ! queue ! ffmpegcolorspace ! queue ! eglglessink sync=false async=false", &error);
*/
    if (error) {
...
static void gst_native_init (JNIEnv* env, jobject thiz) {
  ...
  GST_DEBUG_CATEGORY_INIT (debug_category, "gst_pipeline", 0, "Sonima");
...
static jboolean gst_native_class_init (JNIEnv* env, jclass klass) {
  ...
  if (!custom_data_field_id || !set_message_method_id || !on_gstreamer_initialized_method_id) {
     ...
     */
    __android_log_print (ANDROID_LOG_ERROR, "gst_pipeline", "The calling class does not implement all necessary interface methods");
    return JNI_FALSE;
  }
  return JNI_TRUE;
}
...
jint JNI_OnLoad(JavaVM *vm, void *reserved) {
  ...
  if ((*vm)->GetEnv(vm, (void**) &env, JNI_VERSION_1_4) != JNI_OK) {
    __android_log_print (ANDROID_LOG_ERROR, "gst_pipeline", "Could not retrieve JNIEnv");
    return 0;
  }
  jclass klass = (*env)->FindClass (env, "com/sonima/app/Main");
